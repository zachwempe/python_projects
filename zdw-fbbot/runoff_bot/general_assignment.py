...
sheet_name = 'General Friends Lists'
assignment_sheet_name = 'General Assignments'
...
def build_messenger_list():
    ...
    while msngr_col[0] != '':
        ...
        new_entry = Messenger(msngr_name, msngr_friends, num_msg)
        messenger_list.append(new_entry)
        print(i)
        i += 1
        msngr_col = worksheet.col_values(i)
...
def add_person(column_number):
    ...
    new_person = Messenger(name, friends_list, num_msg)
    assigned_file = open('assigned_people.txt','r')
    assigned_list = assigned_file.readlines()
    assigned_file.close()
    i = 0
